...
def run_mujoco(policy, cfg):
    ...
    if debug_mode:
        ...
        header += [f'target_q_{i}' for i in range(cfg.env.num_actions)]
        header += [f'tau_{i}' for i in range(cfg.env.num_actions)]
    ...
    for _ in tqdm(range(num_steps), desc="Simulating..."):
        ...
        if count_lowlevel % cfg.sim_config.decimation == 0:
            ...
            if debug_mode:
                tau_log = pd_control(target_q, q, cfg.robot_config.kps, target_dq, dq, cfg.robot_config.kds)
                tau_log = np.clip(tau_log, -cfg.robot_config.tau_limit, cfg.robot_config.tau_limit)
                row = [count_lowlevel] + obs[0].tolist() + action.tolist() + q.tolist() + dq.tolist() + target_q.tolist() + tau_log.tolist()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--load_model', type=str, default='/data/wanshan/Desktop/robotics/humanoid-gym/logs/Ironman_ppo/exported/policies/policy_1.pt',
    ...
    class Sim2simCfg(IronmanCfg):
        ...
        class robot_config:
            kps = np.array([40, 20, 20, 40, 40, 40, 20, 20, 40, 40], dtype=np.double)
            kds = np.array([6, 3, 3, 6, 25, 6, 3, 3, 6, 25], dtype=np.double)
